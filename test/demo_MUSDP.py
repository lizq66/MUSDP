...
def run_(X, LOC, n_class, v_lambda, save_path, project_name, model_name, randseed):
    ...
    Popt, CostEffort = rankMeasure.rank_measure(predict_y, test_LOC, test_label)
    measure = [AUC, MCC, Popt, CostEffort]
    fres = create_dir(save_path + model_name + '_label1')
    save_results(fres + project_name, measure)

    # # calculate non-effort-aware classification measure
    AUC, MCC = performanceMeasure.get_measure(test_label, predict_y2)
    # # calculate cost-effectiveness measures
    Popt, CostEffort = rankMeasure.rank_measure(predict_y2, test_LOC, test_label)
    measure2 = [AUC, MCC, Popt, CostEffort]
